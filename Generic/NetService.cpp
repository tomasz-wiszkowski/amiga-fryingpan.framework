...
/* to juz nie wchodzi w czesc generica */
...
void NetService::FConstruct()
    hHkProc(this, &NetService::FSocketProc),
    hHkEvent(this, &NetService::FProcCommands),
    lThreadSignal(0),
    thread("Socket Thread", hHkProc, 0)
{
    thread.SetHandler(hHkEvent.GetHook());
    thread.Start();
}
...
NetService::Socket *NetService::FConnect(char* AHost, int16 APort, const Hook* hDataRdy, const Hook* hWriteRdy, const Hook* hClosed, SocketType stype)
{
    ...
    bsock->setsockopt(lSocket, SOL_SOCKET, SO_EVENTMASK, &lSockEvMask, sizeof(iptr));
...
void NetService::FTryAccept(Socket *skt)
{
	...
	bsock->setsockopt(newskt->socket, SOL_SOCKET, SO_EVENTMASK, &lSockEvMask, sizeof(iptr));
...
int32 NetService::ReadBuffer(NetService::Socket *skt, void* buffer, int32 maxlen)
{
    ...
    return thread.DoSync(Sub_Read, &rb);
}
...
int32 NetService::WriteBuffer(NetService::Socket *skt, void* buffer, int32 maxlen)
{
    ...
    return thread.DoSync(Sub_Write, &wb);
}
...
iptr NetService::FSocketProc(Thread *pThis, void* ASocket)
{
...
iptr NetService::FProcCommands(iptr ACmd, void* AData)
{
    void *Res = 0;
...
NetService::Socket *NetService::Listen(int APort, const Hook* accept, SocketType stype)
{
    ...
    return (Socket*)thread.DoSync(Sub_Listen, &hSub);
}
...
NetService::Socket *NetService::Connect(char* AHost, int APort, const Hook* onData, const Hook* onWriteRdy, const Hook* onClose, SocketType stype)
{
    ...
    return (Socket*)thread.DoSync(Sub_Connect, &hSub);
}
...
void NetService::Disconnect(NetService::Socket *skt)
{
    ...
    thread.DoSync(Sub_Disconnect, &d);
}
