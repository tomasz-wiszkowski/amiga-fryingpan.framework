...
bool NetSocket::FSetupSocket()
{
    ...
    bsock->setsockopt(lSocket, SOL_SOCKET, SO_EVENTMASK, &lSockEvMask, sizeof(uint32));
    if (error < 0)
    {
    ...
    // the following lines make UAE crash badly. it sucks
...
iptr NetSocket::ReadData(void* buffer, iptr maxlen)
{
    return FReadBuffer(buffer, maxlen);
}

iptr NetSocket::WriteData(void* buffer, iptr maxlen)
{
...
int32 NetSocket::FWriteBuffer(void* buff, int32 len)
{
	...
	/*
	   ...
	   {
	   */
	bs = bsock->send(lSocket, &((char*)buff)[sent], len <? 32768, 0);
	/*
	   if (Errno() != 11) // EAGAIN
	   break;
	   DOS->Delay(1);
...
iptr NetSocket::GetDataAvail()
{
    ...
    if (data_timeout > 0)
    {
	stat = WaitStatus(data_timeout, 0, SEv_DataReady | SEv_Disconnected);
	if (0 == (stat & SEv_DataReady))
	    return 0;
    }
...
void NetSocket::SetDataAvailTimeout(iptr to)
{
    data_timeout = to;
}
